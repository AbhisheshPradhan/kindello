...
export function ContinueSearchCard({
  ...
  return (
    ...
    >
      <div style={{ height: 116, position: "relative", background: "#eef2ee" }}>
        <div style={{ position: "absolute", left: 0, right: 0, top: "54%", height: 2, background: "#fff" }} />
        <div style={{ position: "absolute", top: 0, bottom: 0, left: "38%", width: 2, background: "#fff" }} />
        {pins.map(([x, y], i) => (
          <span key={i} style={{ position: "absolute", left: `${x}%`, top: `${y}%`, transform: "translate(-50%,-100%)" }}>
            <span style={{ display: "block", width: 12, height: 12, borderRadius: "var(--radius-pill)", background: "var(--teal-500)", border: "2px solid #fff", boxShadow: "0 1px 3px rgba(0,0,0,.3)" }} />
          </span>
        ))}
      </div>
      <div style={{ padding: "15px 16px 16px", display: "flex", flexDirection: "column", gap: 10, flex: 1 }}>
        ...
        <span style={{ marginTop: "auto", display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 7, padding: 10, borderRadius: "var(--radius-md)", background: "var(--teal-50)", color: "var(--teal-700)", fontSize: 14, fontWeight: 600 }}>
