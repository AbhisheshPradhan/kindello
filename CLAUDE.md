...
- [x] **Full directory frontend on the design system** (`.claude/skills/design-system`): DS tokens ported into `web/app/globals.css`; 15 DS components ported to `web/components/ds/*.tsx`; shared `SiteHeader`/`SiteFooter`/`MapPreview`. Pages: **homepage `/`** (server-rendered marketing — hero + Popular/Browse/Areas/Guides/FAQ off real ACECQA data — with the **AI chat layered on as a client state** via `components/home/*`: Answer tab w/ map preview + floating cards + "Best near" list + follow-ups, Places tab, New search, returning-visitor "Pick up where you left off" + sessionStorage thread persistence); **`/search`** results grid (featured + 3-up `CentreCard`, live URL filter bar); **`/centre/[id]`** SEO detail page (real spine + 7 NQS quality areas + representative programs/features/reviews/FAQ, `generateMetadata`). Reuses the existing `/api/chat` + `lib/tools.ts` backend (added lat/lng + id to `searchCentres` output). Typecheck + prod build clean.
- [ ] **Maps are styled placeholders** (`web/components/ds/map-preview.tsx`) — swap to **Mapbox GL** when `NEXT_PUBLIC_MAPBOX_TOKEN` is set (integration point + bounds maths documented in-file).
