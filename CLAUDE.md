...
**What it is:** a Node/Puppeteer crawler (`ingest/crawl/crawl.js`, run with `node crawl.js --limit N --workers 10`). For each service missing a website it: searches **DuckDuckGo HTML** → picks the centre's own site (filters out directories/social/gov via the `BLOCK` regex) → fetches it → **verifies** the page mentions the centre's name/suburb → scrapes **email** (+ `/contact` fallback, url-decoded), **hero photo** (og:image), and **brand logo** (JSON-LD `logo` → header `img` with "logo" → apple-touch-icon/favicon) → writes to `services_meta`.

**Why this stack (decided, don't relitigate):** everything routes through **Webshare residential proxies** (20 static IPs, `WEBSHARE_PROXIES` in `ingest/.env`, $6/mo 250GB plan) because search engines + some operator WAFs block our datacenter/direct IP. **Puppeteer + stealth + DDG** (not plain HTTP) because plain `requests` gets DDG-rate-blocked AND 403'd by operator WAFs (e.g. Kids Bizz); the stealth browser passes both. A **plain-HTTP Python version was tried and deleted** as too low-yield. Google Places/SERP APIs and Oxylabs were considered and **declined** (keep it free). The crawler uses a **pool of one browser per IP with cross-IP retry**: a row blocked on one IP retries on up to 4 *different* IPs before being deferred; `--workers 10` is the quality/speed sweet spot (20 saturates the laptop and tanks the inspect step). Media (images/css/fonts) is request-blocked to save bandwidth.

**Resumable & safe:** only pulls rows with `discovered_at IS NULL`; a blocked row is left untouched (NOT recorded as a miss) so a re-run retries it on fresh IPs. **Run the full loop, then run it again to mop up blocked rows.** Confined to `services_meta` — never touches the spine or `web/`.

**Coverage check (run this for "what did we scrape"):**
```bash
psql -d kindello -c "SELECT count(*) FILTER (WHERE discovered_at IS NOT NULL) processed, count(website) sites, count(*) FILTER (WHERE website_verified) verified, count(logo_url) logos, count(email) emails, count(photos) photos, count(*) total FROM services_meta;"
```
Fields written: `website`, `website_source` (`'search-ddg'`), `website_verified`, `email`, `logo_url`, `photos` (JSONB `[{url,source:'operator-website',fetched_at}]`). Expected quality when a fetch lands (from sampling): ~60% verified, ~75% logos, ~50% emails, ~38% photos; the gap is centres that genuinely have no own site (OSHC/FDC/small community).

**Open TODO after the crawl completes:** (1) roll centre `logo_url` up to `providers_meta.logo_url`/`website` per brand (dedupe by domain) for the `/brands` pages; (2) quality audit pass (strip obvious false-positive domains the BLOCK list missed); (3) `/contact`-page email coverage could be widened. As of last session: Stage-1 derivables done for all rows; crawler built + validated; ~500 centres enriched; the **full run was handed to the user to run manually** (`caffeinate -i node crawl.js --limit 20000 --workers 10 2>&1 | tee crawl.log`).
