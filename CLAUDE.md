...
- [x] **Suburb page "Search this area" + "Zoom in to search" (BUILT).** The `/childcare/{suburb}/{postcode}` map (and `/{careType}/...` variants) is now interactive like the Finder: panning/zooming offers **"Search this area"** (re-query the map area) or **"Zoom in to search"** below the `MIN_SEARCH_ZOOM` floor (same `AreaPrompt` mechanism in `mapbox-map.tsx`). The re-query is **deterministic** (`getAreaCentres` in `lib/directory.ts` → `/api/directory-area`, returns the same `DirectoryCentre` shape; NOT the AI chat path), and updates the **list, stats chips, and map together** with a "Back to {suburb}" reset. The page's default server render is still the strict suburb set (SEO baseline); area search is a post-load client action. Architecture: `suburb-directory.tsx` is the **server shell** (computes care links / intro / heading), `suburb-results.tsx` is the **client island** that owns the shared list+map state — it `import type`s from `lib/directory` only, so `pg` never reaches the client bundle. _(Note: this is the per-interaction answer to the "Marsfield shows only 3" gap; whether the DEFAULT suburb render should also go radius-based is still the parked/"wait" decision.)_
