...
- [ ] **Finder map — clustering + "Search this area" (PARKED, decided).** The `/finder` map currently caps results at `DEFAULT_CANVAS_LIMIT` (50) nearest the location — kept deliberately simple. Two map features were discussed and parked, to do together later: (1) **"Search this area"** — realestate.com.au-style prompt on user pan/`moveend` to re-query at the new map centre (user-initiated button, not auto; threshold so tiny nudges don't trigger; guard programmatic `fitBounds` moves via `originalEvent`; keep it radius-based with the radius **clamped** to the viewport so zoom-out never "shows everything" — cap + a "showing X of N, zoom in to narrow" facet; gate the button below a sane min-zoom); (2) **clustering** — on zoom-out, collapse overlapping pins into count bubbles. **Decision: use `supercluster` + our existing DOM markers** (NOT native Mapbox GL GeoJSON clustering), to preserve the custom tiered SVG pins, the 4 states, and the React `PinCard` popup. Cluster click → zoom to bounds; co-located leaves at max zoom → open `PinCard` as a **carousel** (`centres[]` + prev/next). Clustering moots the cap (only ~dozens of bubbles ever render) and improves perf, so the 50-cap can rise once it lands.
