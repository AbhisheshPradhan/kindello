...
export default function Home() {
  ...
  const placeholder = isFresh ? PLACEHOLDER_PREFIX + typed : "Write a message…";
    ...
    <div className="flex h-screen flex-col bg-background text-foreground">
      <header className="z-10 flex items-center gap-3 border-b border-border bg-background/80 px-5 py-3 backdrop-blur-md">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold tracking-tight">Kindello</h1>
        </div>
        ...
        <main className="flex flex-1 flex-col items-center justify-center px-4">
          <div className="w-full max-w-2xl text-center">
            ...
            <p className="mt-3 text-muted-foreground">
              Ask in plain English — I&rsquo;ll search by location, care type, and
              quality rating.
            </p>
          ...
          <ChatContainerRoot className="relative flex-1">
            <ChatContainerContent className="mx-auto w-full max-w-3xl px-4 py-6">
              {messages.map((m) => {
                ...
                if (isUser) {
                  return (
                    <div key={m.id} className="flex justify-end py-2">
                ...
                return (
                  <Message key={m.id} className="justify-start py-2">
              ...
              {status === "error" && (
                <div className="py-2 text-sm text-destructive">
                  Something went wrong — check that the selected model&rsquo;s API
                  key is set in <code className="mx-1">.env.local</code> and the
