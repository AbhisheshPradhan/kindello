...
export async function generateMetadata({
  ...
  const type = sp.type ? `${TYPE_LABEL[sp.type] ?? "Childcare"}` : "Approved childcare centres";
...
export default async function SearchPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  return (
    <div style={{ minHeight: "100dvh", display: "flex", flexDirection: "column", background: "var(--bg)" }}>
      ...
      <main style={{ flex: 1 }}>
        {/* Title band */}
        <section style={{ background: "var(--teal-tint)", borderBottom: "1px solid var(--border)" }}>
          <div className="ds-container" style={{ padding: "40px 24px 32px" }}>
            <h1 className="ds-page-h1" style={{ fontWeight: 600, letterSpacing: "-0.02em", color: "var(--fg)" }}>
              {heading}
            </h1>
            <p style={{ fontSize: 15, color: "var(--text-body)", marginTop: 8 }}>
              {total.toLocaleString()} {total === 1 ? "centre" : "centres"} match
              {locationLabel ? `, nearest to ${locationLabel} first` : " across Australia"}. Quality ratings from ACECQA.
            </p>
        ...
        <div className="ds-container" style={{ padding: "36px 24px 64px" }}>
          {centres.length === 0 ? (
            <div style={{ padding: "48px 0", textAlign: "center", color: "var(--muted-fg)" }}>
              No centres matched these filters. Try a different suburb or widen the rating.
            </div>
          ) : (
            <>
              {featured && (
                <div style={{ marginBottom: 28 }}>
                  <CentreCard
                    ...
                    nqs={featured.rating}
                    tags={featured.tags}
                    keyInfo={featured.places != null ? `${featured.places} approved places` : ""}
                    seed={featured.seed}
                  />
                </div>
              )}
