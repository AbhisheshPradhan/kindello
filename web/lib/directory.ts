...
export type DirectoryCentre = {
  ...
  lat: number | null;
  lng: number | null;
  operatingHours: Record<string, Record<string, { start?: string; end?: string }>> | null;
  flags: CareFlags;
...
function careTags(row: Row): string[] {
  ...
  if (row.is_preschool_stand_alone || row.is_preschool_part_of_school) tags.push("Preschool");
  if (row.is_oshc_before_school || row.is_oshc_after_school || row.is_oshc_vacation_care) tags.push("OSHC");
  if (row.service_type === "Family Day Care") tags.push("Family day care");
  if (!tags.length) tags.push("Early learning");
  if (row.number_of_approved_places != null) tags.push(`${row.number_of_approved_places} places`);
...
export function titleCase(s: string): string {
  return s
    ...
    .replace(/\b(Nsw|Vic|Qld|Wa|Sa|Nt|Act|Tas|Oshc|Pcyc|Ymca|Bgs)\b/gi, (m) => m.toUpperCase());
}
...
export type CategoryCount = { key: string; label: string; icon: string; tone: "teal" | "coral" | "sun"; count: number };
...
export async function getCategoryCounts(): Promise<CategoryCount[]> {
  ...
  const r = rows[0];
  return [
    { key: "long_day_care", label: "Long day care", icon: "baby", tone: "teal", count: Number(r.ldc) },
    { key: "family_day_care", label: "Family day care", icon: "users", tone: "coral", count: Number(r.fdc) },
    { key: "preschool", label: "Preschool & kindy", icon: "graduation-cap", tone: "sun", count: Number(r.preschool) },
    { key: "oshc", label: "Outside school hours", icon: "blocks", tone: "teal", count: Number(r.oshc) },
    { key: "", label: "All services", icon: "sun", tone: "coral", count: Number(r.ldc) + Number(r.fdc) },
  ];
}

export async function getTotalCount(): Promise<number> {
  const { rows } = await pool.query<{ n: string }>(`SELECT count(*) n FROM services`);
  return Number(rows[0].n);
}

export async function getCentreById(id: string): Promise<DirectoryCentre | null> {
  const { rows } = await pool.query<Row>(`SELECT ${SELECT} FROM services WHERE service_approval_number = $1`, [id]);
...
export async function getCentreDetail(id: string): Promise<CentreDetail | null> {
  const { rows } = await pool.query<
    Row & {
  ...
  const qualityAreas = QUALITY_AREAS.map((label, i) => ({
    area: i + 1,
    label,
    rating: (r[`quality_area_${i + 1}_rating` as keyof typeof r] as string | null) ?? null,
  }));
  return {
    ...base,
    qualityAreas,
    approvalDate: r.service_approval_granted_date,
    providerName: r.provider_legal_name ? titleCase(r.provider_legal_name) : null,
  };
}

/** Nearby centres for the detail page "related" rail (PostGIS distance). */
export async function getNearbyCentres(centre: DirectoryCentre, limit = 3): Promise<DirectoryCentre[]> {
...
export async function searchDirectory({ suburb, type, minRating, limit = 24 }: SearchParams): Promise<SearchResult> {
  ...
  if (suburb && suburb.trim()) {
    const s = suburb.trim();
    const { rows } = await pool.query<{ state: string; lat: number; lng: number; n: number }>(
  ...
  const where: string[] = ["latitude IS NOT NULL", "service_name IS NOT NULL"];
  const params: unknown[] = [];
  if (type && CARE_PREDICATES[type]) where.push(CARE_PREDICATES[type]);
  if (minRating && RATING_ORDER.includes(minRating as (typeof RATING_ORDER)[number])) {
    const allowed = RATING_ORDER.slice(RATING_ORDER.indexOf(minRating as (typeof RATING_ORDER)[number]));
    params.push(allowed);
    where.push(`overall_rating = ANY($${params.length})`);
  }

  let order = "number_of_approved_places DESC NULLS LAST, service_approval_number";
  if (lat != null && lng != null) {
    params.push(lng, lat);
    where.push(`geog IS NOT NULL AND ST_DWithin(geog, ST_MakePoint($${params.length - 1},$${params.length})::geography, 12000)`);
  ...
  return { centres: rows.map(toCentre), locationLabel, total: Number(cnt[0].n) };
}
