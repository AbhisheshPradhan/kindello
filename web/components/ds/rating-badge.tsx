...
const MAP: Record<string, { label: string; bg?: string; fg?: string; solid?: boolean; base?: string }> = {
  Excellent: { label: "Excellent", bg: "var(--teal-500)", fg: "#fff", solid: true },
  "Exceeding NQS": { label: "Exceeding", base: "var(--teal-500)" },
  "Meeting NQS": { label: "Meeting", base: "var(--rating-meeting)" },
  "Working Towards NQS": { label: "Working towards", base: "var(--rating-working)" },
  "Significant Improvement Required": { label: "Improvement required", base: "var(--rating-improve)" },
};
...
export function RatingBadge({
  ...
  if (!m) {
    css = { background: "var(--secondary)", color: "var(--muted-fg)", boxShadow: "inset 0 0 0 1px var(--border)" };
  } else if (m.solid) {
