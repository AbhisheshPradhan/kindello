...
export function SiteFooter() {
  return (
    <footer style={{ background: "var(--teal-tint)", borderTop: "1px solid var(--border)" }}>
      <div className="ds-container" style={{ padding: "56px 24px 40px" }}>
        <div
          style={{
            display: "grid",
            gap: 40,
            gridTemplateColumns: "minmax(220px, 1.4fr) repeat(2, minmax(120px, 1fr))",
            alignItems: "start",
          }}
        ...
        >
          <div style={{ maxWidth: 320 }}>
            <span style={{ display: "inline-flex", alignItems: "center", gap: 9 }}>
              ...
              <img src="/brand/kindello-mark-clean.png" alt="Kindello" style={{ height: 30, width: "auto" }} />
              <span style={{ fontSize: 21, fontWeight: 600, letterSpacing: "-0.02em", color: "var(--color-primary)" }}>
                Kindello
              </span>
            </span>
            <p style={{ marginTop: 14, fontSize: 14, lineHeight: 1.6, color: "var(--text-body)" }}>
              Every approved childcare and early-education service in Australia, in plain English. Quality ratings
            ...
            {COLS.map((col) => (
              <div key={col.heading}>
                ...
                <ul style={{ listStyle: "none", margin: 0, padding: 0, display: "flex", flexDirection: "column", gap: 9 }}>
                  {col.links.map((l) => (
                    <li key={l.label}>
                      <Link href={l.href} style={{ fontSize: 14, color: "var(--text-body)", textDecoration: "none" }}>
                        {l.label}
                      </Link>
                    </li>
                  ))}
        ...
        >
          <span>© {new Date().getFullYear()} Kindello. Quality ratings sourced from ACECQA.</span>
          <span style={{ display: "inline-flex", gap: 18 }}>
            <Link href="/#about" style={{ color: "var(--muted-fg)", textDecoration: "none" }}>
              Privacy
            </Link>
            <Link href="/#about" style={{ color: "var(--muted-fg)", textDecoration: "none" }}>
              Terms
