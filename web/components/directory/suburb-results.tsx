...
export type CareLink = { href: string; label: string; n: number; active: boolean };
		...
		<div className="grid lg:grid-cols-[1fr_minmax(480px,52%)]">
			...
			<div className="min-w-0 pt-6 pb-12 px-4 sm:px-6 lg:pl-8 lg:pr-6">
				<nav className="text-[12.5px] text-muted-foreground mb-4 flex items-center gap-1.5 flex-wrap">
					...
					<span className="text-body">{data.suburbName}</span>
				</nav>
				...
				{areaMode ? (
					<p className="mt-2 text-[15px] text-muted-foreground leading-relaxed">
						Showing {stats.total}{" "}
						{stats.total === 1 ? "centre" : "centres"} in this map area.{" "}
						<button
							type="button"
				...
				<h2 className="sr-only">Childcare services in {data.suburbName}</h2>
				{centres.length > 0 ? (
