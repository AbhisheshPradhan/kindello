...
export function ResultsToolbar({
	...
	careType: string; // "" = all
	rating: string; // "" = any
	// When the user has run "Search this area", the location reads "Map Area" (a removable
	// chip) instead of the suburb — clearing it restores the suburb view.
	areaActive?: boolean;
	onClearArea?: () => void;
}) {
	...
	const currentLabel = [suburbName, state, postcode]
		.filter(Boolean)
		.join(" ");
		...
		<div className="sticky top-0 z-30 bg-background/95 backdrop-blur border-b border-border">
			<div className="w-full px-4 sm:px-6 lg:px-8 py-3 flex flex-wrap items-center gap-2.5">
				...
				<div className="relative flex-1 min-w-55 max-w-90">
					{areaActive ? (
						<div className="flex items-center gap-1.5 rounded-full border border-border bg-card pl-3 pr-2 py-1.5">
							<Icon
								name="map-pin"
								size={15}
								className="text-teal-500 shrink-0"
							/>
							<span className="inline-flex items-center gap-1.5 rounded-full bg-secondary pl-2.5 pr-1.5 py-0.5 text-[13.5px] font-medium text-body">
								Map Area
								<button
									type="button"
									onClick={onClearArea}
									aria-label="Clear map-area search"
									className="inline-flex items-center justify-center w-4 h-4 rounded-full text-muted-foreground hover:text-foreground hover:bg-border leading-none"
								>
									<span
										aria-hidden
										className="text-[13px]"
