...
const AREAS: { suburb: string; label: string; tone: "teal" | "coral" | "sun" }[] = [
...
function SectionHeader({ title, sub, href }: { title: string; sub?: string; href?: string }) {
  return (
    <div style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", gap: 16, marginBottom: 22 }}>
      <div>
        <h2 className="ds-section-h2" style={{ fontWeight: 600, letterSpacing: "-0.02em", color: "var(--fg)" }}>
          {title}
        </h2>
      ...
      {href && (
        <Link
          href={href}
          style={{ fontSize: 14.5, fontWeight: 600, color: "var(--teal-600)", textDecoration: "none", whiteSpace: "nowrap" }}
        >
          See all →
        </Link>
      )}
...
function FaqItem({ q, a }: { q: string; a: string }) {
  ...
  return (
    <div style={{ borderBottom: "1px solid var(--border)" }}>
      ...
      {open && (
        <p style={{ padding: "0 4px 20px", fontSize: 15, lineHeight: 1.6, color: "var(--text-body)", maxWidth: 720 }}>
          {a}
        </p>
      )}
    </div>
...
export function MarketingSections({
  ...
  return (
    <>
      {/* Popular near you */}
      <section className="ds-container" style={{ padding: "64px 24px" }}>
        <SectionHeader
        ...
        <div className="ds-grid ds-grid-4">
          {popular.map((c) => (
            <CentreCard
              ...
              nqs={c.rating}
              tags={c.tags}
              keyInfo={c.places != null ? `${c.places} approved places` : ""}
              seed={c.seed}
            />
          ))}
        </div>
      ...
      {/* Browse by type */}
      <section style={{ background: "var(--teal-tint)", padding: "64px 0" }} id="browse">
        <div className="ds-container" style={{ padding: "0 24px" }}>
          ...
          <div className="ds-grid ds-grid-5">
            {categories.map((cat) => (
              <CategoryTile
                key={cat.label}
                href={cat.key ? `/search?type=${cat.key}` : "/search"}
                icon={cat.icon}
                label={cat.label}
                count={cat.count}
                tone={cat.tone}
      ...
      {/* Explore by area */}
      <section className="ds-container" style={{ padding: "64px 24px" }}>
        <SectionHeader title="Explore by area" sub="Jump straight to a city or region." />
            ...
            >
              <span
                style={{
                  ...
                  background:
                    a.tone === "teal" ? "var(--teal-50)" : a.tone === "coral" ? "var(--coral-100)" : "var(--sun-100)",
                  color:
                    a.tone === "teal" ? "var(--teal-600)" : a.tone === "coral" ? "var(--coral-500)" : "var(--sun-500)",
                }}
              >
                <Icon name="map-pin" size={22} />
              </span>
              <span>
                <span style={{ display: "block", fontSize: 16, fontWeight: 600, color: "var(--fg)" }}>{a.label}</span>
                <span style={{ fontSize: 13, color: "var(--muted-fg)" }}>Browse centres →</span>
              </span>
            </Link>
          ))}
        </div>
      ...
      {/* Guides */}
      <section style={{ background: "var(--sun-tint)", padding: "64px 0" }} id="guides">
        <div className="ds-container" style={{ padding: "0 24px" }}>
          <h2 className="ds-section-h2" style={{ fontWeight: 600, letterSpacing: "-0.02em", color: "var(--fg)", marginBottom: 22 }}>
            Guides for parents
          </h2>
          <div className="ds-grid ds-grid-3">
            {GUIDES.map((g) => (
              <GuideCard key={g.title} href="/#guides" category={g.category} title={g.title} readTime={g.readTime} seed={g.seed} />
            ))}
          </div>
        </div>
      </section>
      ...
      {/* FAQ */}
      <section className="ds-container" style={{ padding: "64px 24px", maxWidth: 860 }} id="about">
        <h2 className="ds-section-h2" style={{ fontWeight: 600, letterSpacing: "-0.02em", color: "var(--fg)", marginBottom: 8 }}>
