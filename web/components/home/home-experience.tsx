...
const PROMPT_CHIPS = [
  "Long day care near me",
  "Open weekends",
  "Highly rated in the Inner West",
  "Montessori in Surry Hills",
];

const TYPED = [
  "long day care in Parramatta with places now",
  "exceeding-rated kindy near Surry Hills",
  "after-school care around Newcastle",
  "Montessori daycare for a 2 year old in Carlton",
];
...
export function HomeExperience({
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (t) {
        const parsed = JSON.parse(t) as { sessionId: string; messages: typeof messages };
  ...
  useEffect(() => {
    ...
    try {
      if (messages.length) sessionStorage.setItem(THREAD_KEY, JSON.stringify({ sessionId, messages }));
  ...
  function pushHistory(query: string) {
    setHistory((prev) => {
      const next = [{ query, when: Date.now() }, ...prev.filter((h) => h.query !== query)].slice(0, 3);
      try {
  ...
  const placeholderOverlay = useMemo(
    () => (
      <span>
        ...
        <span style={{ animation: "ds-caret-blink 1s step-end infinite", color: "var(--teal-500)" }}>|</span>
      </span>
    ),
    [typed],
  );
    ...
    <div style={{ minHeight: "100dvh", display: "flex", flexDirection: "column", background: "var(--bg)" }}>
      ...
      {isFresh ? (
        <main style={{ flex: 1 }}>
          ...
          <section
            style={{
              background: "linear-gradient(180deg, var(--teal-tint), var(--bg) 86%)",
              padding: "72px 24px 56px",
              textAlign: "center",
            }}
          >
            <div ref={heroRef} style={{ maxWidth: 760, margin: "0 auto" }}>
              <span
                style={{
                  display: "inline-flex",
                  alignItems: "center",
              ...
              >
                ...
                {total.toLocaleString()} approved centres · updated daily
              </span>
              <h1
                className="ds-hero-h1"
                style={{ fontWeight: 600, letterSpacing: "-0.03em", lineHeight: 1.05, color: "var(--fg)" }}
              >
                Find the right childcare, faster.
              </h1>
              <p style={{ fontSize: 19, lineHeight: 1.5, color: "var(--text-body)", margin: "18px auto 0", maxWidth: 580 }}>
                Ask in plain English. Kindello searches every approved service in Australia by location, care type and
                quality rating.
              </p>

              <div style={{ margin: "36px auto 0", textAlign: "left" }}>
                <ChatComposer
                  size="lg"
                  value={input}
                  onChange={setInput}
              ...
              <div style={{ display: "flex", flexWrap: "wrap", gap: 9, justifyContent: "center", marginTop: 20 }}>
                {PROMPT_CHIPS.map((chip) => (
                  <PromptChip key={chip} onClick={() => ask(chip)}>
                    {chip}
                  </PromptChip>
                ))}
              </div>
          ...
          {history.length > 0 && (
            <section className="ds-container" style={{ padding: "8px 24px 8px" }}>
              <h2
                className="ds-section-h2"
                style={{ fontWeight: 600, letterSpacing: "-0.02em", color: "var(--fg)", marginBottom: 18, marginTop: 32 }}
              >
